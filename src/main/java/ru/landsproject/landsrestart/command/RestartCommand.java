...
public class RestartCommand extends BasicCommand {
    ...
    public void run(CommandSender sender, String s, String[] args) {
        if (!sender.hasPermission("landsrestart.restart")) return;
        if (args.length != 1) {
        ...
        if (cooldown > 10) {
            LandsRestart.getInstance().getScheduledPool().scheduleTask(() -> {
                Bukkit.broadcastMessage(LandsRestart.getInstance().getConfiguration().getString("messages.later").replace("<duration>", Messager.getRuTimeForm(10)));
